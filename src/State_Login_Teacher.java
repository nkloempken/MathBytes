...
public class State_Login_Teacher extends State{
     ...
     */
    public State_Login_Teacher(Game game) {
        super(game);
        buttonsAdded = false;
    ...
    public void tick() {
        ...
        if(buttons.get(0).checkButtonHovered(game)&&MouseManager.mousePressed){
            PlayMusic.playSound(game,Assets.buttonClick);
            int result = Users.signIn(usernameBox.getInputString(),passwordBox.getInputString(), "T");
            if(result==1){
                State.setState(new State_Summary_Teacher(game,usernameBox.getInputString()));
                game.setCursor(true);
            }else if (result == 0){
                errorMessage = 0;
            }else{
